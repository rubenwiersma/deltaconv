...
from torch_geometric.transforms import Compose, RandomRotate, RandomTranslate
...
def train(args, writer):
    ...
    # Download ScanObjectNN from https://hkust-vgd.github.io/scanobjectnn/
    # and place in experiments/data/ScanObjectNN/raw.
    # The folder structure in experiments/data/ScanObjectNN should look like:
    # -------------------
    # v ScanObjectNN
    #   v raw
    #     v main_split
    ...
    # Transformations during training: random rotation, jitter, scale, and global translation.
    transform = Compose((
        RandomRotate(360, 1),
    ...
    # Load datasets.
    train_dataset = ScanObjectNN(path, background=args.background, augmentation=args.augmentation, train=True, transform=transform, pre_transform=pre_transform)
    test_dataset = ScanObjectNN(path, background=args.background, augmentation=args.augmentation, train=False, pre_transform=pre_transform)
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4, drop_last=False)
...
if __name__ == "__main__":
    # Parse arguments
    parser = argparse.ArgumentParser(description='DGCNN+ point cloud classification')
    ...
    parser.add_argument('--grad_regularizer', type=float, default=0.01, metavar='lambda',
                        help='Regularizer lambda to use for WLS (default: 0.01)')
    ...
    parser.add_argument('--background', action='store_true',
                        help='Will use dataset with background')
    parser.add_argument('--augmentation', type=str, default=None, metavar='AUG',
                        choices=['PB_T25', 'PB_T25_R', 'PB_T50_R', 'PB_T50_RS'])
    ...
    bg = 'bg' if args.background else 'nobg'
    args.experiment_name = 'scanobjectnn/{}_{}'.format(bg, args.augmentation)
